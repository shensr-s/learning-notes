...
![Spring官网首图](images/image-20200711205546342.png)
...
![Spring Boot的优点](images/image-20200711205612733.png)
...
![Spring Boot与Spring](images/image-20200711205638240.png)
...
![image-20200711205706796](images/image-20200711205706796.png)
...
* ![image-20200711205735129](images/image-20200711205735129.png)
...
![image-20200711205806196](images/image-20200711205806196.png)
...
![编码配置](images/image-20200711205821207.png)
...
![Spring Boot项目工程结构](images/image-20200711205841167.png)
...
然后再定义一下名为 “FILE_PATH” 文件路径，日志都会存储在该路径下。`%i` 表示第 i 个文件，当日志文件达到指定大小时，会将日志生成到新的文件里，这里的 i 就是文件索引，日志文件允许的大小可以设置，下面会讲解。**这里需要注意的是，不管是 windows 系统还是 Linux 系统，日志存储的路径必须要是`绝对路径`。**
...
![使用postman测试表单提交](images/image-20200711205929005.png)
...
![使用Postman测试requestBody](images/image-20200711205953888.png)
...
![官方对swagger的定位](images/image-20200711210014462.png)
...
![image-20200711205424057](images/image-20200711205424057.png)
...
![swagger接口展示](images/image-20200711211046392.png)
...
> 当我们在浏览器中输入 `localhost:8088/thymeleaf/test500` 时，会抛出异常，然后会自动跳转到 500.html 显示。
...
<!--自启动Druid管理后台
不引入该坐标，druid的参数在application.yml文件中会配置出错-->
<dependency>
    <groupId>com.alibaba</groupId>
...
spring:
  ...
  datasource:
    ...
    url: jdbc:mysql://112.74.166.69:3306/course_management?useSSL=false&useUnicode=true&characterEncoding=utf8&useLegacyDatetimeCode=false&serverTimezone=GMT%2B8
    username: root
    password: 123456
    druid:
      max-active: 20
...
```java
package cn.edu.nwafu.fd.config.mp;
...
public class CodeGenerator {
    ...
    public static void main(String[] args) {
        ...
        gc.setOutputDir("D:\\IdeaProjects\\course_fd\\src\\main\\java");
        gc.setAuthor("shensr");
        ...
        dsc.setUrl("jdbc:mysql://127.0.0.1:3306/course_management?useSSL=false&useUnicode=true&characterEncoding=utf8&useLegacyDatetimeCode=false&serverTimezone=GMT%2B8");
        ...
        pc.setParent("cn.edu.nwafu.fd.application");//controller entity  service  service.impl
...
从零开始搭建一个环境，主要要考虑几点：**统一封装的数据结构**、**可调式的接口**、**json的处理**、**模板引擎的使用**（本文不写该项，因为现在大部分项目都前后端分离了，但是考虑到也还有非前后端分离的项目，所以我在源代码里也加上了 thymeleaf）、**持久层的集成**、**拦截器**（这个也是可选的）和**全局异常处理**。一般包括这些东西的话，基本上一个 Spring Boot 项目环境就差不多了，然后就是根据具体情况来扩展了。
...
public class JacksonConfig {
    @Bean
    @Primary //用@Primary 告诉spring 在犹豫的时候优先选择哪一个具体的实现。
    @ConditionalOnMissingBean(ObjectMapper.class)
    public ObjectMapper jacksonObjectMapper(Jackson2ObjectMapperBuilder builder) {
        ObjectMapper objectMapper = builder.createXmlMapper(false).build();
        objectMapper.getSerializerProvider().setNullValueSerializer(new JsonSerializer<Object>() {
